...
object SVMCT {
  def main(args: Array[String]): Unit =
  {
    ...
    */
    ...
    val RegParam: Seq [Double] = Seq (0.10)
    val Tol: Seq[Double] = Seq(1e-4)
    val ElastricNetParam: Seq[Double] = Seq(0.00001)


    val pipeline = new Pipeline()
      .setStages(Array(labelIndexer,assembler, svm))
    ...
    val SVMModel = crossval.fit(trainingData)

    val OptimalenModel = SVMModel.bestModel
    ...
    val SVMTestModel = SVMModel.transform(testingData)
    val EvalMetricResults = SVMTestModel.select("label", "prediction")
    val EvalMetricResultsDF = EvalMetricResults.withColumnRenamed("prediction", "Predicted_Label")
    EvalMetricResultsDF.show(10)

    val predictionAndLabels = SVMTestModel
