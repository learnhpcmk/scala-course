...
object NaiveBayesCT {
  def main(args: Array[String]): Unit =
  {
    ...
    var trainingData = spark.read
      ...
      .load("data/ForestTypes_training_pos.csv")

    var testingData = spark.read
      .option("header","true")
      .option("inferSchema","true")
      .format("com.databricks.spark.csv")
      .load("data/ForestTypes_testing_pos.csv")
    ...
    */
    ...
    val NBModel = crossval.fit(trainingData)

    val OptimalenModel = NBModel.bestModel
    ...
    val EvalMetricResults = NBTestModel.select("label", "prediction", "probability")
