...
object GBTCT {
  def main(args: Array[String]): Unit =
  {
    ...
    var trainingData = spark.read
      ...
      .load("data/ForestTypes_training_svm.csv")

    var testingData = spark.read
      .option("header","true")
      .option("inferSchema","true")
      .format("com.databricks.spark.csv")
      .load("data/ForestTypes_testing_svm.csv")
    ...
    */
    ...
    val pipeline = new Pipeline()
      .setStages(Array(labelIndexer,assembler, gbt))

    var paramGrid = new ParamGridBuilder()
      //.addGrid(gbt.impurity, "gini" :: Nil)
      .addGrid(gbt.maxBins, 5 :: Nil)
      .addGrid(gbt.maxDepth, 5 :: Nil)
      .build()
    ...
    val GBTModel = crossval.fit(trainingData)

    val OptimalenModel = GBTModel.bestModel
    ...
    val forestModel = OptimalenModel.asInstanceOf[org.apache.spark.ml.PipelineModel].stages.last.asInstanceOf[GBTClassificationModel]
    ...
    val GBTTestModel = GBTModel.transform(testingData)
    val EvalMetricResults = GBTTestModel.select("label", "prediction", "probability")
    val EvalMetricResultsDF = EvalMetricResults.withColumnRenamed("prediction", "Predicted_Label")
    EvalMetricResultsDF.show(10)

    val predictionAndLabels = GBTTestModel
