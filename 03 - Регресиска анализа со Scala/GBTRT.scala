...
object GBTRT {
  def main(args: Array[String]): Unit =
  {
    ...
    var paramGrid = new ParamGridBuilder()
      .addGrid(gbt.maxBins, 5 :: 10 :: Nil)
      .addGrid(gbt.maxDepth, 5 :: 10 :: Nil)
      .build()
    ...
    val forestModel = bestModel.asInstanceOf[org.apache.spark.ml.PipelineModel].stages.last.asInstanceOf[GBTRegressionModel]
    ...
    val predictionAndLabels = modelCVResults
      ...
      .asInstanceOf[Double]))
    val GBTRMetrics = new RegressionMetrics(predictionAndLabels)

    val MSE_metrika = GBTRMetrics.meanSquaredError
    val RMSE_metrika = GBTRMetrics.rootMeanSquaredError
    val Rsquared_metrika = GBTRMetrics.r2
    val MAE_metrika = GBTRMetrics.meanAbsoluteError
    val Explained_Variance_metrika = GBTRMetrics.explainedVariance
