...
object GMMCluster {
  def main(args: Array[String]): Unit =
  {
    ...
    val GMMModel = gmm.fit(assemblerDF)

    val GMMClusters = GMMModel.transform(assemblerDF)
    GMMClusters.show()

    GMMClusters.groupBy("prediction").count().show()

    //Evaluacija na Silhouette score
    var evaluator = new ClusteringEvaluator()
    val SilScore = evaluator.evaluate(GMMClusters)
    println("Silhouette Score e = " + SilScore)

    //Prikazi Claster Centrite
    //println("Claster Centrite se ")
    //GMMClusters.clusterCenters.foreach(println)

    var maxSilScore = new Array[Double](10)
    //Evaluate
    for (k <- 2 to 10)
    {
      val SilKModel = new GaussianMixture().setK(k).setSeed(123L)
      val GMMModelEval = SilKModel.fit(assemblerDF)
      val estimatedDF = GMMModelEval.transform(assemblerDF)
